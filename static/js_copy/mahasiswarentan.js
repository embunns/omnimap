...
// ✅ NEW FUNCTION: Populate angkatan dropdown dengan data unik dari mahasiswa
async function populateAngkatanFilter() {
  try {
    ...
    console.error("Error loading angkatan list:", error);
  }
}
...
async function loadMahasiswaData() {
  ...
  try {
    ...
    if (filterAngkatan && filterAngkatan.value) {
      // ✅ Send 2-digit angkatan (22, 23, 24, etc)
...
async function exportMahasiswaData() {
  try {
    ...
    const response = await fetch(`/api/export-mahasiswa-rentan?${params.toString()}`);
